...
如果你把项目部署到阿里云轻量应用服务器，推荐按下面的方式跑：

- 浏览器端继续每 10 分钟请求一次 `/api/quotes`
- `/api/quotes` 在服务端做共享缓存，所有用户共用同一份行情结果
- COMEX 行情缓存 10 分钟，ETF 行情缓存 10 分钟，mock 回退缓存 60 秒
- 同一台服务器上的多个访客不会各自重复打上游行情源
- 上游行情失败时，优先短 TTL 回退，避免页面白屏

这套结构适合单机部署和国内访问场景；如果后面扩展到多台服务器，再考虑把共享缓存迁到 Redis / KV 这类外部存储。
